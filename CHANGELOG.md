...
- **Multi-vault workspaces (preview) — Phase B: union `ls`/`find` views.** With a workspace attached, `xv ls` and `xv find` now span every attached vault instead of just the default:
