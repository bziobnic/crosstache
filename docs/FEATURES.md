...
| `xv schedule install` | Install (or reinstall) a per-user, **target-pinned** job in the OS scheduler (launchd / systemd user timer / Task Scheduler). Resolves and verifies one exact `(config, project/environment, workspace alias, backend instance, real vault)` tuple, writes it to `manifest.json`, and the unit runs `xv schedule run --manifest <path>` — never a vault name or raw `rotate --due --force`. `--interval hourly\|daily\|weekly`, `--at HH:MM`, `--vault` (an attached workspace alias, or a raw vault with no workspace attached), `--log-file`, `--print` to preview the manifest/unit read-only without writing anything, `--force` to skip confirmation. Reinstall is transactional (rollback + `recovery/` on failure) and is the only way to accept a changed target. No daemon; nothing system-wide. See [rotation.md](rotation.md#xv-schedule--run-the-sweep-automatically) |
| `xv schedule status` | Read-only report: ownership (`managed`/`legacy-unpinned`/`orphaned-manifest`/`foreign`), the recorded target (alias, backend/kind, vault, config/project/cwd), drift verdict with every reason, executable (recorded vs. current version), last run (state, times, counts, or `never`/`running since`/`interrupted after`), next run (scheduler-reported or `unknown`), and log presence. Exits `3` when the schedule would refuse its next run or the scheduler could not be queried |
| `xv schedule uninstall` | Remove the native unit(s)/task and `manifest.json`; succeeds when none is installed. Retains `last-run.json`, both lock files, `recovery/`, the rotation log, and any foreign artifact |
