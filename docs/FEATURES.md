...
| `xv update <name> --rename <new>` | Rename a secret on any backend: creates `<new>` with the current value and metadata (tags, groups, note, folder, content type, expiry — not version history), then deletes `<name>` via the backend's normal delete (Azure: soft-deleted; AWS: 30-day recovery window; local: trash). Combined with other update flags, in-place updates apply first, then the rename. Renaming onto an existing name is refused (`xv-conflict`). Partial failure (new secret created, old one not deleted) exits `43` (`xv-rename-incomplete`) and never rolls back the new secret |
