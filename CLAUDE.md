...
- **Web UI**: Embedded localhost browser UI (`xv ui`, `--features ui`) — secret CRUD, folder/group metadata, rename/move, file upload/download, vault switching; loopback-only with a per-session bearer token. A visible tab polls `GET /api/health` (auth-gated like every other route, but deliberately backend-free so an open tab does not put a Key Vault call on a timer) and raises a banner once the `xv ui` process stops answering; a `401` means a *new* process with a new token and is reported separately as an expired session link. Transport state outranks the rail's cached backend-health pill. See `src/web/assets/connection.js`, `docs/web-ui.md`.
