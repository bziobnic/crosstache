...
- **Automatic rotation scheduling**: `xv schedule install|status|uninstall|run` manages a per-user job in the OS scheduler (launchd / systemd user timer / Task Scheduler). No daemon, nothing system-wide. Installs are **target-pinned**: install writes `manifest.json` (config path+digest, `.xv.toml` path/digest/environment, context path/digest, workspace alias/source, backend name/kind/identity digest, real vault, working directory, binary path+version, cadence, log path) under `$XDG_STATE_HOME/xv/schedules/rotation-default/` (`XV_STATE_HOME` overrides on every platform; test/embedding only), and the unit runs `xv schedule run --manifest <path>` — never `rotate --due --force`, never a vault name, never `XDG_CONFIG_HOME`. `HOME` is pinned, plus the state-root variable when one selected the root. `drift::validate_recorded_target` recomputes the target from the recorded inputs before any backend is constructed; refusal exits with the config-error code, and a same-path binary at a new version is a warning only. Install/reinstall/uninstall run under `install.lock` and are transactional (`src/schedule/install.rs`): render in memory → capture prior bytes/registration → publish manifest → write+register → verify byte-exact units *and* the scheduler's own answer → roll back on failure → `recovery/<UTC>-<artifact>` snapshots when rollback itself fails. Ownership (`src/schedule/ownership.rs`) is decided from bytes: the `Managed by crosstache (xv schedule)` marker plus the command shape give `managed` / `legacy-unpinned` / `orphaned-manifest` / `foreign` / absent, and a scheduler command failure is `SchedulerState::Error`, never absence
(`SchedulerState` lives in `ownership.rs`, where it is produced). Ownership and
registration are independent: a `managed` unit the scheduler answers `Absent`
for renders `Scheduler: not registered` and is an `[error]` exiting `3` (`The
<scheduler> rotation schedule is not registered.`), never `[ok]`; the
`Scheduler:` line is printed for any non-absent ownership whenever the probe is
not `Installed`. Legacy (pre-manifest) units are replaced only by an explicit `xv schedule install` — no automatic migration. `uninstall` removes only the owned unit(s)/task and `manifest.json`, retaining `last-run.json`, `run.lock`, `install.lock`, `recovery/`, the log, unrelated files and foreign artifacts. Lifecycle logic is tested against a fake `CommandRunner` and a fake store, and CLI tests set `XV_SCHEDULE_RUNNER=fake` (+ `XV_SCHEDULE_RUNNER_LOG=<path>`; `fake:registered` additionally answers the install transaction's own verification queries out of what it was asked to register, so one CLI test drives the real transaction end to end), honored only in debug builds via `src/schedule/testing.rs` — no test touches the real scheduler, whose job name is a fixed global label `HOME` does not sandbox. Because that switch is compiled out of a release build, every scheduler-touching CLI test self-skips under `cargo test --release` (`skip_if_release!()`, backed by a `scheduler_output` guard that refuses to spawn at all). `schedule run` acquires a nonblocking `run.lock` (`fs2::FileExt`) before validation; a second concurrent firing logs `schedule rotation skipped: another run is already_running` to stderr, exits 0, and leaves `last-run.json` untouched. Every run writes a bounded `last-run.json` (`RunState`: `running`/`success`/`partial_failure`/`failed`/`refused_drift`; a `refused_drift` or unreadable-manifest run exits with the configuration-error code `3`) with a ≤1024-scalar sanitized diagnostic and no secret names/values; `xv schedule status` renders it as `Last run:`, labels an outcome bound to a stale manifest digest `(previous install)`, and reports a live `running` record as `running since <ts>` or, once the lock is free again, `interrupted after <ts> (no runner holds the lock)`. Status also parses the scheduler's own next-run time (`Next run:`, `unknown` when the platform does not expose one, or exposes one that names no zone — schtasks' bare local wall clock is refused rather than stamped with the host's current UTC offset, which is wrong across a DST boundary) and renders the full seven-dimension block (ownership, schedule, target, backend, config/project/cwd, drift, executable, last/next run, log). See `src/schedule/{manifest,target,drift,install,ownership,outcome,status,status_render,testing}.rs`, `src/cli/schedule_ops.rs`, `docs/rotation.md`.
