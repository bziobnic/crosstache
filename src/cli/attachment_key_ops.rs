...
pub enum AttachmentKeyCommands {
    ...
    Rewrap {
        #[arg(long)]
        vault: Option<String>,
        /// Expected current V2 active key ID
        #[arg(long)]
        to_key_id: String,
        #[command(flatten)]
        action: ApplyOptions,
...
pub(crate) async fn execute(command: AttachmentKeyCommands, config: Config) -> Result<()> {
    ...
    match command {
        ...
        AttachmentKeyCommands::Rewrap { action, .. } => {
            ...
            let report = attachment_rewrap::rewrap(
                backend.attachment_keys().as_ref(),
                files,
                &vault,
                rewrap_id.as_ref().expect("validated rewrap ID"),
                action.apply,
            )
            .await?;
            render(
...
mod backup_cli_tests {
    ...
    fn rewrap_cli_requires_expected_target_and_offline_apply() {
        assert!(Cli::try_parse_from(["xv", "attachment-key", "rewrap"]).is_err());
        let args = ["xv", "attachment-key", "rewrap", "--to-key-id", "ak1-test"];
        assert!(Cli::try_parse_from(args).is_ok());
        assert!(Cli::try_parse_from(args.into_iter().chain(["--apply"])).is_err());
        assert!(Cli::try_parse_from(args.into_iter().chain(["--apply", "--offline"])).is_ok());
    }
