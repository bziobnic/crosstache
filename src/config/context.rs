...
impl VaultContext {
    ...
    pub fn new(vault_name: String, resource_group: Option<String>, subscription_id: Option<String>) -> Self {
        Self {
            vault_name,
            resource_group,
            subscription_id,
...
impl ContextManager {
    ...
    async fn load_local_context() -> Result<Self> {
        ...
        debug!("Loaded local context from: {}", context.context_file.as_ref().unwrap().display());
        Ok(context)
    }
    ...
    async fn load_global_context() -> Result<Self> {
        ...
        debug!("Loaded global context from: {}", context.context_file.as_ref().unwrap().display());
        Ok(context)
    }
    ...
    pub fn current_resource_group(&self) -> Option<&str> {
        self.current.as_ref().and_then(|c| c.resource_group.as_deref())
    }
    
    /// Get current subscription ID from context
    pub fn current_subscription_id(&self) -> Option<&str> {
        self.current.as_ref().and_then(|c| c.subscription_id.as_deref())
    }
    ...
    pub async fn migrate_from_config(
        ...
        info!("Migrated default vault '{}' to context system", default_vault);
        Ok(context_manager)
    }
