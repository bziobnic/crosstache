...
pub async fn diagnose_and_repair(path: &Path) -> Result<DoctorReport> {
        ...
        Err(toml_error) => match serde_json::from_str::<Value>(text) {
            ...
            Err(_) => {
                return Ok(report.with_unresolved(format!(
                    "TOML syntax error in '{}': {toml_error}",
                    path.display()
                )));
            }
        },
    ...
    let config = match parsed {
        ParsedConfig::Toml(document) => {
            toml::from_str::<Config>(&document.to_string()).map_err(|error| error.to_string())
        }
        ParsedConfig::Json(value) => {
            serde_json::from_value::<Config>(value).map_err(|error| error.to_string())
        }
    };

    match config {
        Ok(_) => Ok(report.with_check(
            DoctorCheckStatus::Ok,
            format!("Configuration file '{}' is valid.", path.display()),
        )),
        Err(error) => Ok(report.with_unresolved(format!(
            "Configuration schema error in '{}': {error}",
            path.display()
        ))),
    }
}
