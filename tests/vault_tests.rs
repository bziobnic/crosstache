...
mod vault_validation_tests {
    ...
    fn test_vault_name_validation() {
        ...
        for name in invalid_names {
            assert!(!is_valid_vault_name(name), "Name '{}' should be invalid", name);
        }
    }
    ...
    fn test_location_validation() {
        ...
        for location in valid_locations {
            assert!(is_valid_azure_location(location), "Location '{}' should be valid", location);
        }
        ...
        for location in invalid_locations {
            assert!(!is_valid_azure_location(location), "Location '{}' should be invalid", location);
        }
    }
    ...
    fn test_retention_days_validation() {
        ...
        for days in valid_days {
            assert!(is_valid_retention_days(days), "Retention days {} should be valid", days);
        }
        ...
        for days in invalid_days {
            assert!(!is_valid_retention_days(days), "Retention days {} should be invalid", days);
        }
    }
}
...
mod vault_uri_tests {
    ...
    fn test_invalid_vault_uri() {
        ...
        for uri in invalid_uris {
            assert!(extract_vault_name_from_uri(uri).is_err(), "URI '{}' should be invalid", uri);
        }
    }
}
...
mod vault_resource_tests {
    ...
    fn test_resource_group_validation() {
        ...
        for name in valid_names {
            assert!(is_valid_resource_group_name(name), "Resource group '{}' should be valid", name);
        }
        
        // Test max length separately
        let max_length_name = "a".repeat(90);
        assert!(is_valid_resource_group_name(&max_length_name), "90 char name should be valid");
        ...
        for name in invalid_names {
            assert!(!is_valid_resource_group_name(name), "Resource group '{}' should be invalid", name);
        }
        
        // Test too long name separately
        let too_long_name = "a".repeat(91);
        assert!(!is_valid_resource_group_name(&too_long_name), "91 char name should be invalid");
    }
    ...
    fn test_subscription_id_validation() {
        ...
        for id in valid_ids {
            assert!(is_valid_subscription_id(id), "Subscription ID '{}' should be valid", id);
        }
        ...
        for id in invalid_ids {
            assert!(!is_valid_subscription_id(id), "Subscription ID '{}' should be invalid", id);
        }
    }
}
...
fn is_valid_azure_location(location: &str) -> bool {
    let valid_locations = vec![
        ...
        "northeurope", "westeurope", "francecentral", "germanywestcentral", "norwayeast", "switzerlandnorth",
        "uksouth", "ukwest", "southeastasia", "eastasia", "australiaeast", "australiasoutheast",
        "brazilsouth", "canadacentral", "canadaeast", "centralindia", "southindia", "westindia",
        "japaneast", "japanwest", "koreacentral", "koreasouth", "southafricanorth", "uaenorth"
    ];
...
fn extract_vault_name_from_uri(uri: &str) -> Result<String, String> {
    ...
    let start = "https://".len();
    let end = uri.find(".vault.azure.net").ok_or("Invalid vault URI format")?;
